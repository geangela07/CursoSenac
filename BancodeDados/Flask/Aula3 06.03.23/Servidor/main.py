...
def verPoekmonEspecifico(id_pokemon):
    idPokemon = id_pokemon
    resultado = con.consultarBanco(f'''
    SELECT * FROM "Pokemons" WHERE "Id" = '{idPokemon}'
    ''')
    if "Ocorreu um erro" in resultado:
        return resultado
    elif resultado == []:
        return "Não há pokemon com este id"
    else:
        return jsonify(resultado)
    
@app.route("/Pokemons/<int:id_pokemon>/update", methods =["Post"])
def atualizarPokemonEspecifico(id_pokemon):
    try:
        idPokemon = id_pokemon
...
@app.route("/Pokemons/<int:id_pokemon>/delete", methods = ["Port"])
def deletarPokemonEspecifico(id_pokemon):
    idPokemon = id_pokemon
